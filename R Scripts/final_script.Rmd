...
- X and id: These columns seem to represent some unique identifier for each observation. X appears to be an integer index, while id is also an integer and likely represents a customer ID or some form of identifier.

- Gender: This column seems to contain information about the gender of the passengers, with values such as "Male" and "Female."

- Customer.Type: This variable describes the type of customer, with categories like "Loyal Customer" and "disloyal Customer."

- Age: Represents the age of the passengers and is an integer variable.

- Type.of.Travel: Indicates the purpose of travel, with categories like "Personal Travel" and "Business travel."

- Class: Specifies the class of travel, with categories like "Eco Plus" and "Business."

- Flight.Distance: This variable contains the distance of the flight in miles as an integer.

- Inflight.wifi.service, Departure.Arrival.time.convenient, and several other columns: These variables seem to represent passengers' ratings or feedback on different aspects of their flight experience. They are integer variables with ratings ranging from 1 to 5.

- Departure.Delay.in.Minutes and Arrival.Delay.in.Minutes: These columns appear to represent the delay in minutes for departure and arrival, respectively. Departure delay is an integer, while arrival delay is a numeric variable.

- satisfaction: This is the target variable or the outcome of interest, and it appears to represent customer satisfaction levels with values like "neutral or dissatisfied" and "satisfied."
...
dim(data)
```
It is a data frame with 103,904 observations (rows) and 25 variables (columns)
...
4. **Age**:
   - The age variable ranges from 7 to 85 with a mean age of approximately 39.38.

5. **Type of Travel**:
   - There are two types of travel: 'Business travel' (69.0%) and 'Personal Travel' (31.0%). Business travel is the more common type.
...
7. **Flight Distance**:
   - The mean flight distance is approximately 1189 miles, ranging from 175 to 3383 miles.
...
table_1 <- kableExtra::scroll_box(xkabledply(missing_data , title = "Missing Values — Initial",
  pos = "left",
  bso = "striped"), width = "100%", height = "100%")
table_1
```
...
table_2 <- kableExtra::scroll_box(xkabledply(missing_data , title = "Missing Values — Final",
  pos = "left",
  bso = "striped"), width = "100%", height = "100%")
table_2
```
