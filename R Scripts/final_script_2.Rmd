...
dim(data)
```
This is the initial description of data
...
The table here shows the NA values in our dataset
```{r result_missing,echo=FALSE}
xkabledply(missing_data , title = "Missing Values")
```

Now let's replace these 310 NA values in arrival delays with their median we are going to see this in EDA because it has a skewed distribution so we are using median.
...
We have removed all the missing values and replaced them with the median of the data as shown in table below
...
```
These are some summary statistics for the numeric variables
```{r}
summary_stats_numeric
```

These are some summary statistics for the categorical variables
```{r}
...
The following graphs depict frequency distributions for continuous variables in our dataset
...
## Frequency distribution for ordinal variables (Ratings)
...
*Arrival/Departure Delays:* It is difficult to discern any meaningful differences between passengers that were satisfied or neutral/dissatisfied based on arrival or departure delay durations.


***

## Visualize the relationship between Arrival and Departue delays colored by satisfaction.
...
Here we can see their is high correlation between arrival and departure delays
...
At first glance in our initial EDA, many of the ratings appear to share similar frequency distributions, sparking multicollinearity concerns. Our next step to evaluate these potential relationships was to create a correlation matrix.
...
```{r ratings_matrix_final, echo=FALSE}
