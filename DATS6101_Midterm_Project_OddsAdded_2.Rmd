...
# Get working directory, grabbing parent folder path to access the csv
# Import train.csv data as a data frame
data_raw <- data.frame(read.csv("C:\\Users\\jehan\\DATS6101\\Midterm\\data\\train.csv", header = TRUE))
...
A critical step in evaluating categorical variables is to encode them for use in modeling. Encoding was conducted as follows:
...
<br>
```{r discrete, include = FALSE}

# This chunk transforms continuous variables into discrete ones for use in linearity checks
...
Unlike a standard linear regression, which assumes that independent parameters have a linear relationship with the dependent variable, **logistic regression assumes that parameters have a linear relationship with the log odds**. In a similar manner, ordinal variables are expected to be related in such a manner that each increment has an equivalent effect on the log odds.
...
log_odds_Class <- aggregate(satisfaction ~ Class, data, log_odds_calc)

describe(log_odds_Class)
...
```{r logistic_model_init, echo = FALSE}

logistic_model_init <- glm(satisfaction ~ Age + In_Flight_and_Baggage_Ratings + Flight.Distance + Gender + Customer.Type + Type.of.Travel + Class, data = data, family = binomial)

summary(logistic_model_init)


```
```{r logistic_model_final, echo = FALSE}

logistic_model_final <- glm(satisfaction ~ Age + In_Flight_and_Baggage_Ratings + Customer.Type + Type.of.Travel + Class, data = data, family = binomial)

summary(logistic_model_final)

car::vif(logistic_model_final)
